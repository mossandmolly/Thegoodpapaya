// Supabase Edge Function — CSV order upload
//
// Input:  { password, records, phones? }
//   records: [{ customer_name, invoice_date, item_name, requested_quantity, description?, community? }]
//   phones:  { "CustomerName": "9876543210" }  — supplied when re-submitting after missingPhones response
//
// Output: { inserted, held, date }
//   held = items that already existed in operations (duplicate — needs admin review)
//
// Behaviour:
//   • Generates sales_order_id = invoice_date + '-' + customer_name (spaces → dashes)
//   • Upserts one orders header row per customer (no cart, source='csv')
//   • Merges into operations: new items → 'draft'; duplicate (sales_order_id+item_name) → 'held'
//   • Existing packing data is never overwritten
//   • Phones saved to customer_phones (not customers.phone_number)
//
// Required env vars:
...
// Fetch known phones from customer_phones table
...
// Fetch existing operations keys for a given date to detect duplicates
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Save any newly provided phones to customer_phones
    if (newPhones.length) {
    ...
    // ── Fetch existing operations keys to detect duplicates ───────────────────
    const existingKeys = await fetchExistingKeys(uploadDate);

    // ── Separate new vs duplicate items ───────────────────────────────────────
    const newOps:  any[] = [];
    const heldIds: string[] = [];  // sales_order_id+item_name combos that already exist
    ...
    // ── Mark duplicate items as 'held' ────────────────────────────────────────
    // Update status to 'held' only if not already in 'final' state (preserve packing work)
    for (const key of heldIds) {
      const [soid, itemName] = key.split('||');
      const encoded          = encodeURIComponent(itemName);
      await sbFetch(
        `operations?sales_order_id=eq.${soid}&item_name=eq.${encoded}&status=eq.draft`,
    ...
    return new Response(
      JSON.stringify({ inserted: newOps.length, held: heldIds.length, date: uploadDate }),
      { headers: { ...CORS, 'Content-Type': 'application/json' } },
    );
